...
let runtimeGeminiKey =
  process.env.GEMINI_API_KEY || "AIzaSyBR2Rx861b6lmzQ49y1n7CavNt4Ai9JZC8";
let runtimeOpenaiKey =
  process.env.OPENAI_API_KEY ||
  "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";
...
let runtimeJimengBaseUrl =
  process.env.JIMENG_BASE_URL || "https://ark.cn-beijing.volces.com/api/v3";
let runtimeKlingKey = process.env.KLING_API_KEY || "";
let runtimeKlingBaseUrl =
  process.env.KLING_BASE_URL || "https://api.klingai.com/v1";
let runtimeDeepseekKey = process.env.DEEPSEEK_API_KEY || "";
let runtimeDeepseekBaseUrl =
...
let runtimeWhitelist = (process.env.IP_WHITELIST || "127.0.0.1,::1")
  .split(",")
  .map((ip) => ip.trim());
...
try {
  ...
  if (fs.existsSync(CONFIG_PATH)) {
    ...
    if (savedConfig.runtimeOpenaiKey)
      runtimeOpenaiKey = savedConfig.runtimeOpenaiKey;
    if (savedConfig.runtimeOpenaiBaseUrl !== undefined)
      runtimeOpenaiBaseUrl = savedConfig.runtimeOpenaiBaseUrl;
    if (savedConfig.runtimeMidjourneyKey)
      runtimeMidjourneyKey = savedConfig.runtimeMidjourneyKey;
    if (savedConfig.runtimeMidjourneyBaseUrl !== undefined)
      runtimeMidjourneyBaseUrl = savedConfig.runtimeMidjourneyBaseUrl;
    if (savedConfig.runtimeFluxKey) runtimeFluxKey = savedConfig.runtimeFluxKey;
    if (savedConfig.runtimeFluxBaseUrl !== undefined)
      runtimeFluxBaseUrl = savedConfig.runtimeFluxBaseUrl;
    if (savedConfig.runtimeJimengKey)
      runtimeJimengKey = savedConfig.runtimeJimengKey;
    if (savedConfig.runtimeJimengBaseUrl !== undefined)
      runtimeJimengBaseUrl = savedConfig.runtimeJimengBaseUrl;
    if (savedConfig.runtimeKlingKey)
      runtimeKlingKey = savedConfig.runtimeKlingKey;
    if (savedConfig.runtimeKlingBaseUrl !== undefined)
      runtimeKlingBaseUrl = savedConfig.runtimeKlingBaseUrl;
    if (savedConfig.runtimeDeepseekKey)
      runtimeDeepseekKey = savedConfig.runtimeDeepseekKey;
    if (savedConfig.runtimeDeepseekBaseUrl !== undefined)
      runtimeDeepseekBaseUrl = savedConfig.runtimeDeepseekBaseUrl;
    if (savedConfig.runtimeWhitelist)
      runtimeWhitelist = savedConfig.runtimeWhitelist;
...
function getAI(): GoogleGenAI {
  ...
  if (!keyToUse) {
    throw new Error(
      "GEMINI_API_KEY is not set. 鉴权失败：网关控制面板尚未配置底层 GEMINI_API_KEY。",
    );
  }
...
function mapModelName(model: string): string {
  ...
  if (model === "jimeng-5.0" || model === "doubao-seedream-5.0-lite")
    return "ep-20260511162417-9xd8q";
  if (model === "jimeng-4.5" || model === "doubao-seedream-4.5")
    return "ep-20260511162504-fjd6v";
...
// 辅助函数：判断是否为即梦/豆包模型
function isVolcengineImageModel(
  ...
  return (
    (originalModel.includes("jimeng") ||
      originalModel.includes("doubao") ||
      originalModel.includes("seedream")) &&
    !originalModel.includes("video") &&
    !originalModel.includes("seedance")
  );
}

function isVolcengineVideoModel(
  originalModel: string,
  actionType?: string,
): boolean {
  if (actionType === "jimeng-video") return true;
  return (
    (originalModel.includes("jimeng") && originalModel.includes("video")) ||
    originalModel.includes("seedance")
  );
}
...
const globalLimiter = rateLimit({
  ...
  message: {
    ...
    detail:
      "网关探测到您的源 IP 请求频率异常。请降低并发请求速度，1分钟后自动解封。",
  },
...
const heavyLimiter = rateLimit({
  ...
  message: {
    ...
    detail:
      "生成图像或视频过于频繁。请控制排队分发策略，限制单用户每分钟的生成次数以免消耗过度。",
  },
});
...
function mediaConcurrencyLimiter(
  req: express.Request,
...
try {
  ...
  console.log("Failed to load previous billing logs.");
}
...
try {
  ...
  console.log("Failed to load previous dispatch logs.");
}
...
function recordDispatchLog(
  ...
  const cleanPayload = (obj: any): any => {
    ...
    if (typeof obj === "string") {
      return obj.length > 2000
        ? obj.substring(0, 100) + "...[TRUNCATED_LONG_STRING]"
        : obj;
    }
    if (typeof obj === "object") {
      ...
      for (const [k, v] of Object.entries(obj)) {
        if (
          ...
          cleaned[k] =
            typeof v === "string"
              ? v.substring(0, 50) + "...[BASE64_IMAGE_TRUNCATED]"
              : v;
        } else if (
          k === "url" &&
          typeof v === "string" &&
  ...
  saveLogsToFile();
  console.log(
    `[调度记录] IP: ${log.ip} | 接口: ${endpoint} | 模型: ${log.model} | 结果: ${success ? "成功" : "失败"} | 信息: ${message}`,
  );
}
  ...
  try {
    if (apiType === "chat") {
      ...
      if (model.includes("gemini-3.1-pro")) {
        ...
        cost = (inTokens / 1000000) * 1.25 + (outTokens / 1000000) * 5.0;
      } else if (model.includes("gpt-4o-mini")) {
        // GPT-4o-mini: $0.150 / 1M input, $0.600 / 1M output
        cost = (inTokens / 1000000) * 0.15 + (outTokens / 1000000) * 0.6;
      } else if (model.includes("gpt-4o")) {
        // GPT-4o: $2.50 / 1M input, $10.00 / 1M output (Updated pricing)
        cost = (inTokens / 1000000) * 2.5 + (outTokens / 1000000) * 10.0;
      }
...
function requireAuth(
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    console.log(
      `[Auth Failed] Missing or invalid Authorization header from ${getRealClientIP(req)}`,
    );
    return res
      .status(401)
      .json({
        success: false,
        error: "未授权：请提供有效的 Authorization Bearer token。",
      });
  }
  const token = authHeader.split(" ")[1];
  if (token !== runtimeProxySecret) {
    console.log(
      `[Auth Failed] Token mismatch from ${getRealClientIP(req)}. Provided: ${token ? "***" + token.slice(-3) : "none"}`,
    );
    return res
      .status(403)
      .json({
        success: false,
        error: "鉴权未通过：私有锁口令 (PROXY_SECRET_KEY) 不正确。",
      });
  }
  next();
}
...
app.use("/api", (req, res, next) => {
  ...
  if (prompt && !isContentSafe(prompt)) {
    return res
      ...
      .json({
        error:
          "请求被拒绝：包含不合规的内容词汇。(Content Rejected by Moderation)",
      });
  }
  next();
});
...
app.get("/api/health", (req, res) => {
  res.json({
    status: "ok",
    message: "海外 API 代理服务运行正常 (API Proxy is running)",
  });
});
...
app.get("/api/admin/config", requireAuth, (req, res) => {
  res.json({
    ...
    geminiKey: runtimeGeminiKey
      ...
      : "",
    openaiKey: runtimeOpenaiKey
      ? `${runtimeOpenaiKey.substring(0, 4)}****${runtimeOpenaiKey.substring(runtimeOpenaiKey.length - 4)}`
      : "",
    openaiBaseUrl: runtimeOpenaiBaseUrl,
    midjourneyKey: runtimeMidjourneyKey
      ? `${runtimeMidjourneyKey.substring(0, 4)}****${runtimeMidjourneyKey.substring(runtimeMidjourneyKey.length - 4)}`
      : "",
    midjourneyBaseUrl: runtimeMidjourneyBaseUrl,
    fluxKey: runtimeFluxKey
      ? `${runtimeFluxKey.substring(0, 4)}****${runtimeFluxKey.substring(runtimeFluxKey.length - 4)}`
      : "",
    fluxBaseUrl: runtimeFluxBaseUrl,
    jimengKey: runtimeJimengKey
      ? `${runtimeJimengKey.substring(0, 4)}****${runtimeJimengKey.substring(runtimeJimengKey.length - 4)}`
      : "",
    jimengBaseUrl: runtimeJimengBaseUrl,
    klingKey: runtimeKlingKey
      ? `${runtimeKlingKey.substring(0, 4)}****${runtimeKlingKey.substring(runtimeKlingKey.length - 4)}`
      : "",
    klingBaseUrl: runtimeKlingBaseUrl,
    deepseekKey: runtimeDeepseekKey
      ? `${runtimeDeepseekKey.substring(0, 4)}****${runtimeDeepseekKey.substring(runtimeDeepseekKey.length - 4)}`
      : "",
...
app.post("/api/admin/config", requireAuth, (req, res) => {
  console.log(
    `[Admin] Received config update request from ${getRealClientIP(req)}`,
  );
  const {
    whitelist,
    geminiKey,
  ...
  if (openaiBaseUrl !== undefined) {
    // Base URL can be empty to fallback to default
    ...
    try {
      ...
      if (!prompt || !model)
        return res
          .status(400)
          .json({
            error: "缺少 prompt 或 model 参数 (Missing prompt or model)",
          });
      ...
      if (rawSize.match(/^\d+x\d+$/)) {
        targetSize = rawSize;
        if (rawSize.includes("2048") || rawSize.includes("1536"))
          geminiImageSize = "2K";
        else if (rawSize.includes("4096") || rawSize.includes("2304"))
          geminiImageSize = "4K";
        else if (rawSize.includes("512") || rawSize.includes("256"))
          geminiImageSize = "512";
        else geminiImageSize = "1K";
      } else {
        if (rawSize.includes("512") || rawSize === "small") {
          targetSize = "512x512";
          geminiImageSize = "512";
        } else if (rawSize.includes("2048") || rawSize === "2k") {
          targetSize = "2048x2048";
          geminiImageSize = "2K";
        } else if (rawSize.includes("4096") || rawSize === "4k") {
      ...
      console.log(
        `[Unified Gateway] Auto-routing request for model: ${model}, targetGeminiSize: ${geminiImageSize}`,
      );
      ...
      if (model.includes("sora")) {
        ...
        const videoUrl =
          data.data && data.data[0] ? data.data[0].url : data.url || null;
      ...
      if (
        ...
        if (isMidjourney && runtimeMjMode === "task") {
          console.log(
            "[Unified Gateway] MJ Proxy Task Mode Active. Submitting imagine task...",
          );
          const submitResponse = await fetch(`${BASE_URL}/mj/submit/imagine`, {
            ...
            body: JSON.stringify({
              prompt:
                prompt +
                (req.body.aspectRatio ? ` --ar ${req.body.aspectRatio}` : ""),
              base64Array: req.body.referenceImage
          ...
          if (!submitResponse.ok || submitData.code !== 1) {
            throw new Error(
              `MJ Proxy 提交失败: ${submitData.description || "未知错误"}`,
            );
          }

          const taskId = submitData.result;
          console.log(
            `[Unified Gateway] Task submitted successfully: ${taskId}. Waiting for completion...`,
          );
          ...
          while (Date.now() - startTime < TIMEOUT) {
            ...
            if (fetchData.status === "SUCCESS" && fetchData.imageUrl) {
              ...
              throw new Error(
                `MJ Proxy 生成失败: ${fetchData.failReason || "任务执行错误"}`,
              );
            }
            console.log(
              `[Unified Gateway] Task ${taskId} status: ${fetchData.status}...`,
            );
          }

          if (!finalUrl)
            throw new Error("MJ Proxy 任务超时或未返回图片链接 (Task Timeout)");
        ...
        if (
          (model.includes("midjourney") || model.includes("mj")) &&
      ...
      if (
        isVolcengineImageModel(req.body.model, req.body.actionType) ||
        (model.startsWith("ep-") &&
          !req.body.model.includes("video") &&
          !req.body.model.includes("seedance") &&
          !req.body.model.includes("dance") &&
          req.body.actionType !== "jimeng-video")
      ) {
        const API_KEY = runtimeJimengKey;
        let BASE_URL =
          runtimeJimengBaseUrl || "https://ark.cn-beijing.volces.com/api/v3";
        BASE_URL = BASE_URL.trim().replace(/\/$/, "");
        if (
          BASE_URL.includes("ark.cn-beijing.volces.com") &&
          !BASE_URL.includes("/api/v3")
        ) {
          BASE_URL = `${BASE_URL}/api/v3`;
        }
        if (!API_KEY)
          throw new Error(`网关未配置火山引擎 (即梦/豆包) 所需的 API Key`);

        let jimengMappedSize = targetSize;

        const originalModel = req.body.model || "";
        if (
          originalModel.includes("5-0") ||
          originalModel.includes("5.0") ||
          originalModel.includes("4-5") ||
          originalModel.includes("4.5") ||
          model === "ep-20260511162417-9xd8q" ||
          model === "ep-20260511162504-fjd6v"
        ) {
      ...
      if (
        ...
        isVolcengineVideoModel(req.body.model, req.body.actionType) ||
        (model.startsWith("ep-") &&
          (req.body.model.includes("video") ||
            req.body.model.includes("seedance") ||
            req.body.model.includes("dance") ||
        ...
        if (
          rawKlingUrl.includes("klingai.com") &&
          !rawKlingUrl.includes("/v1")
        ) {
        ...
        if (
          rawJimengUrl.includes("ark.cn-beijing.volces.com") &&
          !rawJimengUrl.includes("/api/v3")
        ) {
        ...
        if (!API_KEY)
          throw new Error(
            `网关未配置 ${isKling ? "可灵 (KLING)" : "即梦 (JIMENG)"} 所需的 API Key`,
          );
        ...
        if (isKling && API_KEY) {
          ...
          if (
            parts.length === 2 &&
            !API_KEY.startsWith("eyJ") &&
            !API_KEY.startsWith("sk-") &&
            BASE_URL.includes("kling")
          ) {
        ...
        if (!submitResponse.ok)
          throw new Error(
            submitData.error?.message || JSON.stringify(submitData),
          );
        ...
        while (Date.now() - startTime < TIMEOUT) {
          ...
          try {
            ...
            console.error(
              `[Unified Gateway] Polling JSON error on ${pollUrl}: ${pollText.substring(0, 100)}...`,
            );
            if (!pollResponse.ok) continue; // Just wait, maybe 502/HTML is intermittent
            throw new Error(
              `上游服务器解析错误 (Upstream format error): ${pollText.substring(0, 50)}`,
            );
          }
      ...
      if (model.includes("imagen") || model.includes("image")) {
        ...
        return res.json({
          success: true,
          imageUrl: rawBase64 ? `data:image/jpeg;base64,${rawBase64}` : null,
          estimatedCostUsd,
        });
      }
      ...
      const estimatedCostUsd = recordBillingLog(
        ...
        model,
        prompt,
        { inputTokens: promptTokens, outputTokens: candidatesTokens },
        req,
      );
      ...
      res
        .status(500)
        .json({ success: false, error: error.message || "内部服务错误" });
    }
  },
);
...
app.get("/api/chat", (req, res) => {
  res
    ...
    .json({
      ...
      message:
        "这是一个 POST 接口，不能在浏览器里直接访问（浏览器默认是 GET）。请用代码发 POST 请求。",
    });
});
app.get("/api/image", (req, res) => {
  res
    ...
    .json({
      error: "方法不允许 (Method Not Allowed)",
      message:
        "这是一个 POST 接口，不能在浏览器里直接访问。请用代码发 POST 请求。",
    });
});
    ...
    try {
      ...
      if (model && model.includes("gemini")) {
        ...
        const estimatedCostUsd = recordBillingLog(
          "chat",
          model,
          "Standard /v1/chat/completions call",
      ...
      if (!API_KEY)
        throw new Error(
          `网关未配置 ${model} 所需的 API Key (未提供对应平台的 Key)`,
        );
      ...
      console.error(
        "[OpenAI Compatible API] v1/chat/completions Error:",
        error,
      );
      recordDispatchLog(
        false,
...
app.get(
  ["/v1/models", "/api/v1/models", "/v1/v1/models"],
  requireAuth,
  async (req, res) => {
    res.json({
      object: "list",
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (rawSize.includes("2048") || rawSize === "2k") geminiImageSize = "2K";
        ...
        geminiImageSize = "4K";
      else if (rawSize.includes("512") || rawSize === "small")
        geminiImageSize = "512";
      else geminiImageSize = "1K";
      ...
      if (model && model.includes("gemini")) {
        const ai = getAI();
        let reqAspectRatio =
          req.body.aspectRatio || req.body.aspect_ratio || "1:1";
        if (
          !req.body.aspectRatio &&
        ...
        const refImg =
          req.body.referenceImage || req.body.image || req.body.init_image;
        if (refImg) {
          let finalData = "";
      ...
      if (
        ...
        while (Date.now() - startTime < TIMEOUT) {
          ...
          try {
            ...
            console.error(
              `[Unified Gateway - OpenAI] Polling JSON error on ${pollUrl}: ${pollText.substring(0, 100)}...`,
            );
        ...
        recordDispatchLog(
          ...
          req.path,
          model,
          "Video API success returning as Image API format",
          req.body,
          req,
          resultData,
        );
      ...
      if (
        ...
        if (
          refImg &&
          typeof refImg === "string" &&
          refImg.startsWith("data:")
        ) {
      ...
      console.error(
        "[OpenAI Compatible API] v1/images/generations Error:",
        error,
      );
      recordDispatchLog(
        false,
...
app.post("/api/chat", requireAuth, async (req, res) => {
  try {
    ...
    if (!prompt)
      return res
        .status(400)
        .json({ error: "缺少 prompt 参数 (Missing prompt)" });
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `[API] Image Generation requesting model: ${model}, aspect: ${aspectRatio}`,
      );
      ...
      if (
        ...
        if (
          ...
          if (
            jimengMappedSize === "1024x1024" ||
            jimengMappedSize === "512x512"
          ) {
        ...
        const estimatedCostUsd = recordBillingLog(
          ...
          model,
          prompt,
          { imageCount: 1, resolution: reqBody.size },
          req,
        );
        recordDispatchLog(
          true,
      ...
      const estimatedCostUsd = recordBillingLog(
        ...
        model,
        prompt,
        { imageCount: numberOfImages, resolution: imageSize },
        req,
      );
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `[API] Started video generation operation: ${operation.name}. Waiting for completion...`,
      );
  ...
  try {
    ...
    if (!key) {
      return res
        .status(400)
        .json({
          error: `网关未配置 ${model} 所需的 API Key。请在【控制面板】填写此密钥。`,
        });
    }
...
app.all("/v1/*", (req, res) => {
  ...
  res.status(404).json({
    error: {
      message: `Invalid URL (POST ${req.path})`,
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(
      `🚀 Overseas API Server proxy running on http://localhost:${PORT}`,
    );
  });
}
